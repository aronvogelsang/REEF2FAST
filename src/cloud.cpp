...
WavefieldByTime read_wavefield_csv(const std::string& filename, double z_max) {
    ...
    std::cout << "Wavefield: " << total << " points loaded across " << data.size() << " timesteps.\n";
    return data;
}
...
void generate_seastate_grid_targets(const std::string& control_file,
    ...
    std::cout << "REEF3D Grid Domain Size:\n";
    ...
    std::cout << "  Total: "  << targets.size() << " grid points\n";
}    
...
InterpolationResult interpolate_full_wavefield(const std::string& wavefield_file,
    ...
    WavefieldByTime wavefield_by_time = read_wavefield_csv(wavefield_file, z_max);

    std::vector<std::array<double, 3>> target_points;
    generate_seastate_grid_targets(control_file, target_points);
    ...
    auto vx_interp = interpolate_to_grid(wf, target_points, "vx");
    auto vy_interp = interpolate_to_grid(wf, target_points, "vy");
    auto vz_interp = interpolate_to_grid(wf, target_points, "vz");
    auto p_interp  = interpolate_to_grid(wf, target_points, "pressure");
    ...
    pt[0], pt[1], pt[2],
    vx_interp[i], vy_interp[i], vz_interp[i],
    p_interp[i], NAN,  // elevation
    NAN, NAN, NAN      // acceleration
    });
    }

    // Optional: max diagnostics
    double max_vx = -1e10, max_vy = -1e10, max_vz = -1e10, max_p = -1e10;
